...
//---------------------------------------------------------------------------------------
// bounding box for a ellipse (http://iquilezles.org/www/articles/ellipses/ellipses.htm)
//---------------------------------------------------------------------------------------
bound3 EllipseAABB( in vec3 c, in vec3 u, in vec3 v )  // disk: center, 1st axis, 2nd axis
{
    vec3 e = sqrt( u*u + v*v );
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3  disk_u = 0.3*sin(iTime*vec3(1.3,1.1,1.2)+vec3(0.5,0.0,2.0));
    vec3  disk_v = 0.3*sin(iTime*vec3(1.0,1.2,1.1)+vec3(4.0,2.0,1.0));

    // render
   	vec3 col = vec3(0.4)*(1.0-0.3*length(p));

    // raytrace disk
    float t = iEllipse( ro, rd, disk_center, disk_u, disk_v );
	float tmin = 1e10;
    if( t>0.0 )
	{
    	tmin = t;
