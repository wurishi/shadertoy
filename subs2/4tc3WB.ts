...
uniform bool u_seamless_loop;

uniform bool u_colour_cycle;
...
let gui: GUI;
const api = {
  u_seamless_loop: true,
  u_colour_cycle: false,
};
...
export default class implements iSub {
  ...
  main(): HTMLCanvasElement {
    gui = new GUI();
    gui.add(api, 'u_seamless_loop');
    gui.add(api, 'u_colour_cycle');
    return createCanvas();
  }
  userFragment(): string {
    return fragment;
  ...
  initial?(gl: WebGLRenderingContext, program: WebGLProgram): Function {
    const u_seamless_loop = webglUtils.getUniformLocation(
      gl,
      program,
      'u_seamless_loop'
    );
    const u_colour_cycle = webglUtils.getUniformLocation(
      gl,
      program,
      'u_colour_cycle'
    );
    return () => {
      u_seamless_loop.uniform1i(api.u_seamless_loop ? 1 : 0);
      u_colour_cycle.uniform1i(api.u_colour_cycle ? 1 : 0);
    };
  }
}
