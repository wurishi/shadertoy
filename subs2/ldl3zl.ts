...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    for( int n=ZERO; n<AA; n++ )
    {
        ...
        for( int j=ZERO; j<jlen; j++ )
        {
            // raymarch
            vec4 info;
            float t = intersect(rq,rd,info);
            if(u_reflections && info.x<-0.5) {
              break;
            }
            ...
            if(u_reflections) {
              ...
              rq = pos + nor*0.01;
              rd = normalize(ref + 0.02*sin(o.x+o.y*7.0+vec3(0.0,1.0,2.0)));
            }
        }
...
export default class implements iSub {
  ...
  initial?(gl: WebGLRenderingContext, program: WebGLProgram): Function {
    const u_reflections = webglUtils.getUniformLocation(
      gl,
      program,
      'u_reflections'
    );
    return () => {
      u_reflections.uniform1i(api.u_reflections ? 1 : 0);
    };
  }
  channels() {
