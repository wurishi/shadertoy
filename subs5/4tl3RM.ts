...
vec3 CubeMap(in sampler2D sam, in vec3 v, float size)
{
   float M = max(max(abs(v.x), abs(v.y)), abs(v.z));
   float scale = (float(size) - 1.) / float(size);
   if (abs(v.x) != M) v.x *= scale;
...
vec3 DoLighting(in vec3 mat, in vec3 pos, in vec3 normal, in vec3 eyeDir, in float d, in float specular)
{
    ...
    col += pow(max(dot(sunLight, normal), 0.0), 1.0)  * SUN_COLOUR* texture(iChannel3, normal.xy).xyz * specular *sh;
	return col;
}
