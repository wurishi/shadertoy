...
float p0c(int x,int y){
    vec2 p = gl_FragCoord.xy+vec2(x,y);
	return texture(iChannel2,p/iResolution.xy).x;
}
...
{
    vec2 r = iResolution.xy;   
    float pl = texture(iChannel2,(C-vec2(-1, 0))/r).x;
    float pr = texture(iChannel2,(C-vec2( 1, 0))/r).x;
    float pt = texture(iChannel2,(C-vec2( 0,-1))/r).x;
    float pb = texture(iChannel2,(C-vec2( 0, 1))/r).x;
    vec2 grad = vec2(pr-pl,pb-pt)/2.;
    vec4 bufOld = texture(iChannel0,C/r);
...
void mainImage( out vec4 fragColor, in vec2 C )
{
    ...
    if(length(uv-m)<0.02){
    	...
    	fragColor = vec4(texture(iChannel3,C/r).z);
    }
    fragColor.a = 1.;
}
`;
...
export default class implements iSub {
  ...
  name(): string {
    return 'Multistep Fluid Simulation';
  }
  // sort() {
  //   return 0;
  // }
