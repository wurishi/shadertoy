...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    vec2 uv = fragCoord.xy / iResolution.xy;
        ...
        vec2 d   = texture(iChannel0, uv).xy;
        vec2 d_n = texture(iChannel0, fract(uv+n)).xy;
        vec2 d_e = texture(iChannel0, fract(uv+e)).xy;
        vec2 d_s = texture(iChannel0, fract(uv+s)).xy;
        vec2 d_w = texture(iChannel0, fract(uv+w)).xy;    

        vec3 i   = texture(iChannel2, fract(uv + DISP * d  ), MIP).xyz;
        vec3 i_n = texture(iChannel2, fract(uv + DISP * d_n), MIP).xyz;
        vec3 i_e = texture(iChannel2, fract(uv + DISP * d_e), MIP).xyz;
        vec3 i_s = texture(iChannel2, fract(uv + DISP * d_s), MIP).xyz;
        vec3 i_w = texture(iChannel2, fract(uv + DISP * d_w), MIP).xyz;
