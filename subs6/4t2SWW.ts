...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	if (d<maxd)
	{
		...
		ray = reflect(rd, n);
		cubeRay = texture(iChannel0, ray.xy).rgb  * refl_i ;

		ray = refract(ray, n, refr_a);
		cubeRay += texture(iChannel0, ray.xy).rgb * refr_i;
	...
	{
		col = texture(iChannel0, rd.xy).rgb;
	}
...
export default class implements iSub {
  ...
  name(): string {
    return 'Ray Marching Experiment n°35';
  }
  sort() {
    return 684;
  }
