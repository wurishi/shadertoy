...
{
    float Spots = 1.5 * texture(iChannel2, 3.0 * UV).x;
...
{    
    float Skin = 1.0 - mix(texture(iChannel2, 1.0 * Pos.xy).x, texture(iChannel2, 2.0 * Pos.xy).x, 0.3);
    float Stone = mix(texture(iChannel2, 0.5 * Pos.xy).x, texture(iChannel2, 1.3 * Pos.xy).x, 0.3);
...
vec3 EyeballTexture(vec2 EyeUV)
{
    ...
    float BloodNoise = pow(0.3 * texture(iChannel2, 3.0 * EyeUV).x + 0.7 * texture(iChannel2, 1.0 * EyeUV).x, 0.5);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    {                
        ...
        IrisYellowMask = max(IrisYellowMask, smoothstep(0.3, 0.9, texture(iChannel2, 5.0 * IrisUV).x));
        
        vec3 IrisBaseColor = vec3(IrisTexture(IrisUV-0.5)) * mix(IrisGreen, IrisYellow, IrisYellowMask);

        float IrisBlackSpotMask = smoothstep(0.055, 0.045, length(IrisUV-0.5)) * smoothstep(0.4, 1.0, texture(iChannel2, 20.0 * IrisUV).x);
        ...
        float SkinSpots = 0.0;
        SkinSpots = max(SkinSpots, texture(iChannel1, 1.7 * (Pos.xy + 0.11)).x);
        SkinSpots = max(SkinSpots, texture(iChannel1, 1.1 * (Pos.xy + 0.17)).x);
        SkinSpots = max(SkinSpots, texture(iChannel1, 0.3 * (Pos.xy + 0.79)).x);
        ...
        vec3 StoneBaseColor = mix(
            vec3(0.05), 
            vec3(0.67, 0.57, 0.40), 
            pow(texture(iChannel3, 0.5 * Pos.xy + vec2(0.61, 0.49)).xxx, vec3(2.2)));
        // 改
        // vec3 StoneBaseColor = vec3(0.0, 1.0, 0.0);
        
        float Edge0 = StoneFadeTime + length(Pos) - 2.0;
        float Edge1 = Edge0 - 0.02;
        float StoneMaskTex = mix(texture(iChannel2, 0.5 * Pos.xy + 0.5).x, texture(iChannel3, 0.5 * Pos.xy + 0.5).x, 0.5);
        ...
        // 改
        vec3 Env = pow(2.0 * texture(iChannel3, Reflection.xy).xxx, vec3(2.2));
