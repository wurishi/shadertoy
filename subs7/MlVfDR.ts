...
const buffB = `// Voronoi based particle tracking

vec2 R;float N;
vec4 T ( vec2 U ) {return texture(iChannel2,U/R);}
vec4 P ( vec2 U ) {return texture(iChannel3,U/R);}
...
{	R = iResolution.xy;
	...
	vec2 
 	...
 	float d = (length(n-C.xy)-1.+
        ...
        length(s-C.xy)-1.+
        length(w-C.xy)-1.);
 	float m1 = 2.*texture(iChannel3,vec2(abs(0.3*C.w),0.)).x,
 	      m2 = 1.5*texture(iChannel3,vec2(abs(0.3*C.z),0.)).x;
