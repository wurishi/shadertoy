...
const float nebRadius = 2.72;
const vec3 nebCenter = vec3(0.,nebRadius,0.);
...
vec4 raymarch( in vec3 rayo, in vec3 rayd, in float nebInter, in vec2 fragCoord )
{
	...
	vec3 pos = rayo + rayd * (nebInter + step*texture( iChannel0, fragCoord.xy/iChannelResolution[0].x ).x);
